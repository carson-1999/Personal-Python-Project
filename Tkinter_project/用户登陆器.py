...
import pickle  #数据序列化及反序列化存储和传输，以二进制的形式，dump()是序列化，load()是反序列化为python数据类型,这个模块的读写操作都要二进制读取

# 第1步，实例化object，建立窗口window
window = tk.Tk()

# 第2步，给窗口的可视化起名字
window.title('Welcome to Carson \'s Website')

# 第3步，设定窗口的大小(长 * 宽)
window.geometry('400x300')  # 这里的乘是小x

# 第4步，加载 wellcome image
canvas = tk.Canvas(window, width=400, height=135, bg='green') #创建指定大小的画布
image_file_path = tk.PhotoImage(file='1.jpg')  #利用PhotoImage类加载图片路径
image = canvas.create_image(200, 0, anchor='n', image=image_file_path)  #用image属性将图片路径加载至画布
canvas.pack(side='top')

tk.Label(window, text='Welcome', font=('Arial', 16)).pack() #在画布下创建一个标签显示信息
...
# 第8步，定义用户登录功能
def usr_login():
    # 这两行代码就是获取用户输入的usr_name和usr_pwd,通过返回变量的get()方法获取，一般也可以entry名.get()获输入框的内容
